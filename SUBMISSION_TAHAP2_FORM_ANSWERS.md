# Jawaban Form — Submission Tahap 2 (hackathon.pidi.id/profile → Tahap 2)

> Tim: **404 Inovators** · Proposal: **PULSA** · Disusun dari `PROPOSAL_TAHAP2.md` versi final (sudah dengan perbaikan alignment Inklusi Ekonomi UMKM, POJK 3/2024, label asumtif).
>
> **Cara pakai:** copy isi tiap blok "Jawaban" ke field form yang sama namanya. Form punya **32 pertanyaan wajib** (tanda *) + lampiran. Teks sudah *paste-ready* (tabel dilinearkan jadi teks polos — textarea form tidak me-render markdown).
>
> ⚠️ Verifikasi sebelum submit: setiap jawaban sudah lolos batas kata panduan (cek dengan `count_words.py`). Survei masih N=7 (belum diupdate, sesuai keputusan — perlu data tambahan).

---

## 1. ID Tim *
*Masukkan ID Tim Anda sesuai dengan data pendaftaran.*

**Jawaban:**
```
P0670
```
> (Diambil dari halaman Profil akun hackathon Anda.)
...
**Jawaban:**
```
Beni Handoko — Product Lead / Full-Stack Engineer: strategi produk, arsitektur sistem, full-stack development, validasi UMKM.
Moch Arsya Adzma Syahputra — Software Engineer / Data Engineer: backend, ingestion pipeline, analytics engine, implementasi Business Health Score.
...
(Lihat lampiran: data_flow_diagram.png — diagram alur input → processing → analytics & AI → output.)
```
...
- Revenue Stability (bobot 25%): 1 − Coefficient of Variation harian, normalisasi terhadap median industri.
- Transaction Frequency (bobot 20%): rasio transaksi/hari vs benchmark sektor (F&B, retail, jasa).
- Customer Diversity (bobot 15%): distribusi unique customer, indeks konsentrasi (Herfindahl-like).
- Channel Diversity (bobot 15%): jumlah channel aktif + distribusi revenue per channel.
- Growth Trend (bobot 25%): slope regresi linear revenue 30 hari, normalisasi industri.
...
(Diagram arsitektur lengkap: lampiran architecture_diagram.png)
```
...
```
> ⚠️ Catatan internal (jangan dimasukkan ke form): bagian survei masih N=7. Sesuai kesepakatan, ini belum diupdate — perlu data tambahan (target N≥30) untuk memperkuat kredibilitas sebelum/ sesudah submit.
...
**Link Attachment (URL) — saran:**
```
https://handokobeni.github.io/pulsa-demo/
```

**File Attachment (jika pakai PDF):**
- Format wajib **PDF**, maksimal **5MB**.
- Aturan penamaan file: **`P0670 - PULSA: AI Financial Intelligence untuk UMKM Berbasis Data Transaksi`**
- Isi PDF disarankan: proposal final + 7 screenshot prototype (`attachments/01_login.png` … `07_ingestion.png`) + diagram (`architecture_diagram.png`, `data_flow_diagram.png`).

---

## ✅ Checklist sebelum Submit (32/32 wajib)
- [ ] ID Tim **P0670** terisi (sebelumnya placeholder di proposal — sudah ketemu dari halaman Profil)
- [ ] Semua 32 field wajib terisi (form menampilkan counter "x/32 Pertanyaan Wajib Terjawab")
- [ ] Cek tiap jawaban tidak melebihi batas kata panduan (`python3 count_words.py`)
- [ ] Lampiran: minimal isi **Link** demo, atau upload PDF dengan penamaan `P0670 - <Judul>`
- [ ] **Disarankan simpan "Save as Draft" dulu**, review tampilan, baru **Submit**
- [ ] ⚠️ Saya TIDAK meng-submit form atas nama Anda — submit final dilakukan Anda sendiri
